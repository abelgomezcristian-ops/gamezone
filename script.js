...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const platformInfo = {
    ps2:{label:"CATÁLOGO PS2",title:"Juegos PS2",placeholder:"Buscar en PS2...",icon:"🎮"},
    ps4:{label:"CATÁLOGO PS4",title:"Juegos PS4",placeholder:"Buscar en PS4...",icon:"🎮"},
    ps5:{label:"CATÁLOGO PS5",title:"Juegos PS5",placeholder:"Buscar en PS5...",icon:"🎮"},
    switch:{label:"CATÁLOGO SWITCH",title:"Juegos Switch",placeholder:"Buscar en Switch...",icon:"🕹️"}
  };
  ...
  function createProductCard(game){
    ...
    return `<article class="product-card"><div class="product-image"><div class="product-badges">${badges}</div>${img}</div><div class="product-info"><span class="product-genre">${escapeHTML(game.genre||'PS2')}</span><h3 class="product-title">${escapeHTML(game.name)}</h3><div class="product-bottom"><span style="font-size:11px;color:#9da3b5">Consultar</span><button class="add-cart-button" data-id="${game.id}">Consultar</button></div></div></article>`;
  }

  function getFiltered(){
    if(currentConsole!== "ps2") return [];
    let f = games;
    if(currentSearch){
      const q = currentSearch.toLowerCase();
      f = f.filter(g => g.name.toLowerCase().includes(q) || (g.genre && g.genre.toLowerCase().includes(q)));
    }
    return f;
  }
  ...
  function renderProducts(){
    ...
    if(currentConsole!== "ps2"){
      productsGrid.innerHTML = "";
      paginationEl.innerHTML = "";
      platformComing.hidden = false;
      const info = platformInfo[currentConsole];
      platformComing.innerHTML = `<div><div style="font-size:64px;margin-bottom:12px">${info.icon}</div><h3>${info.title} — Próximamente</h3><p style="color:#9da3b5;margin-top:8px">Estamos cargando el catálogo de ${info.title}.<br>Consultanos por WhatsApp que te avisamos.</p><br><button class="secondary-button" style="padding:10px 18px;border:1px solid rgba(255,255,255,0.15);border-radius:10px;background:#1e2235;color:#fff;font-weight:800" onclick="document.querySelector('[data-console=ps2]').click()">Volver a PS2</button></div>`;
      return;
    }
    ...
    if(filtered.length===0){
      productsGrid.innerHTML = `<div style="grid-column:1/-1;padding:40px;text-align:center;color:#9da3b5">No encontramos "${escapeHTML(currentSearch)}" en PS2</div>`;
      paginationEl.innerHTML="";
      return;
    }
  ...
  function updateCartUI(){
    ...
    cartItems.innerHTML = cart.map(id=>{
      ...
      return `<div class="cart-item"><span>${escapeHTML(g.name)}</span><button data-del="${id}" style="width:26px;height:26px;border-radius:50%;border:0;background:#222;color:#fff">×</button></div>`;
    }).join("");
  ...
  // BOLITAS FLOTANTES - CADA UNA CON SU BUSCADOR
  consoleBubbles.forEach(bubble=>{
    bubble.addEventListener("click",()=>{
      ...
      currentSearch = "";
      searchInput.value = "";
      const info = platformInfo[consoleType];
      catalogLabel.textContent = info.label;
      catalogTitle.textContent = info.title;
      searchInput.placeholder = info.placeholder;
      renderProducts();
      if(consoleType==="ps2") document.getElementById("juegos").scrollIntoView({behavior:"smooth"});
